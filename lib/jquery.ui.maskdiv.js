(function() {
  ...
  $.widget("ui.maskDiv", {
    ...
    _move: function(change_position) {
      ...
      var el = div.find("[data-num=" + this.current_position + "]").focus();
      this._set_cursor(el);
      return el;
    },
    _replace_current_text_and_move: function() {
      var mask_string, opts, el, new_text;
      opts = this.options;
      mask_string = _.flatten(opts.mask).join('');
      new_text = mask_string[this.current_position];
      this._move().text(new_text).removeClass(opts.edited_char_class).addClass(opts.unedited_char_class);
    },
    ...
    _add_range: function(selection, node) {
      ...
      if (el[0]) {
        if (window.getSelection) {
          if (window.getSelection().setPosition) {
            ...
            return this._add_range(window.getSelection(), el[0]);
          }
        }
      } else if (document.selection) {
        return this._add_range(document.selection, el[0]);
      }
      return;
    },
    _handle_blur_mask: function(e) {
      ...
      div.html(el_text.slice(0, ((el_text.length - 1 - last_match_index) * -1)).join(''));
